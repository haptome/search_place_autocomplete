part of search_place_autocomplete;

class SearchPlaceAutoCompleteWidget extends StatefulWidget {
  ...
  ///
  /// Check the full list of [supported  country code](Wikipedia: List of ISO 3166 country codes or the ISO Online Browsing Platform) in the documentation.
  final String components;

  @override
  _SearchPlaceAutoCompleteWidgetState createState() => _SearchPlaceAutoCompleteWidgetState();
}

class _SearchPlaceAutoCompleteWidgetState extends State<SearchPlaceAutoCompleteWidget> with TickerProviderStateMixin {
        ...
        children: <Widget>[
          ...
          if (widget.hasClearButton)
            GestureDetector(
              onTap: () {
                if (_crossFadeState ==
                    CrossFadeState.showSecond) _textEditingController.clear();
              },
  ...
  */
  ...
  Future<dynamic> _makeRequest(input) async {
    String url =
        ...
        .apiKey}&language=${widget.language}&components=country:${widget.components}";


    if (widget.location != null && widget.radius != null) {
      url += "&location=${widget.location.latitude},${widget.location
          .longitude}&radius=${widget.radius}";
  ...
  void _closeSearch() async {
    if (!_animationController.isDismissed) await _animationController.animateTo(
        0.5);
    _fn.unfocus();
    setState(() {
      _placePredictions = [];
